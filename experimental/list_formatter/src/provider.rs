...
impl<'a> ConditionalListJoinerPattern<'a> {
    ...
    /// exact, otherwise it's is an upper bound (as currently special cases only occur
    /// in the end and pair positions, meaning they are only used once, using an upper
    /// bound here won't waste much memory, but guarantees that the string doesn't have
...
pub(crate) mod test {
    ...
    pub fn test_patterns() -> ListFormatterPatternsV1<'static> {
        ListFormatterPatternsV1([
            ...
            // Narrow: conditionals
            ConditionalListJoinerPattern::from_str("{0}: {1}").unwrap(), // Same as wide
            ConditionalListJoinerPattern::from_str("{0}, {1}").unwrap(), // Same as wide
    ...
    fn size_hint_works() {
        ...
        assert_eq!(pattern.size_hint(Width::Short, 2), 6); // pair pattern {0}123{1}456
        assert_eq!(pattern.size_hint(Width::Short, 200), 399); // ": " + 197 * ", " + " :o "
