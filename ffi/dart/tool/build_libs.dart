...
Future<Uri> buildLib(
  ...
  bool buildStatic,
  bool isSimulator,
  FileSystemEntity startingPoint,
  ...
  var root = startingPoint is Directory ? startingPoint : startingPoint.parent;
  while (!File.fromUri(root.uri.resolve('Cargo.toml')).existsSync()) {
    root = root.parent;
  }
  ...
  if (!isNoStd) {
    ...
    await runProcess('rustup', rustArguments, workingDirectory: root.path);
  }

  await runProcess('cargo', [
    if (buildStatic || isNoStd) '+nightly',
    'rustc',
    '--manifest-path=${root.path}/ffi/capi/Cargo.toml',
