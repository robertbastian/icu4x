    ...
    fn load_resource(
        ...
        let era_names: cldr_serde::ca::Resource =
            serde_json::from_reader(open_reader(&self.era_names_path)?)
                .map_err(|e| Error::Json(e, Some(self.era_names_path.clone())))?;
        let era_dates: cldr_serde::japanese::Resource =
            serde_json::from_reader(open_reader(&self.era_dates_path)?)
                .map_err(|e| Error::Json(e, Some(self.era_dates_path.clone())))?;
        ...
        if env::var("ICU4X_SKIP_JAPANESE_INTEGRITY_CHECK").is_err() {
            ...
            if snapshot != ret {
                return Err(DataError::custom("Era data has changed! This can be for two reasons: Either the CLDR locale data for Japanese eras has \
