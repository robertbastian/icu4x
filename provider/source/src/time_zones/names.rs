...
fn test_normalize_canonicalize_coverage() {
    ...
    for (iana_id, bcp47_id) in iana2bcp.iter() {
        let canonicalized = mapper.canonicalize(iana_id).unwrap().0;
